...
| 4 | 매칭 차수 수정 가능 | `PATCH` | `200 OK` 또는 빈 응답 |
...
> 수정 요청의 `chapterId`는 Command에 반영되지 않습니다. 즉, 매칭 차수가 소속된 지부는 수정할 수 없고, 권한 및 기간 중복 검증은 기존 매칭 차수의 `chapterId` 기준으로 수행됩니다.
...
| `endsAt` | `2026-05-22T00:00:00Z` |
| `decisionDeadline` | `2026-05-23T00:00:00Z` |
| `chapterId` | 기존 매칭 차수의 `chapterId`, 요청 바디의 `999`로 변경되면 안 됨 |
