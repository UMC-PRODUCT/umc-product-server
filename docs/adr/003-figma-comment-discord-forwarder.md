...
Amended (2026-05-07): 4·5·7번 결정을 LLM 분류 + 도메인 라우팅 + Discord embed 포맷 + LLM 도메인 분리로 변경. 자세한 사유는 아래 본문 및 `Alternatives Considered §3` 의 "결정 번복 사유" 참조.
...
5. **멘션 + 메시지 포맷 (Amended)**: 도메인 단위로 멘션 대상 (Discord role 또는 user) 을 N개까지 영속화(`figma_routing_domain_mention`)한다. 메시지는 평문이 아니라 **Discord embed** 로 포맷되며, 멘션은 알림 발생을 위해 외부 `content` 영역에, 가독성 정보(파일/도메인/페이지/작성자/본문/링크/시각)는 embed 로 분리한다.
...
### Discord 메시지 포맷 (Amended — embed)

```jsonc
{
  "content": "<@&123> <@456>",   // 멘션만, 알림 발생 영역
  "embeds": [{
    "title": "[Figma] {파일이름} 새 댓글",
    "description": "{댓글 본문}",
    "url": "https://www.figma.com/file/{fileKey}?node-id={nodeId}#{commentId}",
    "color": 15879710,            // 0xF24E1E (Figma 브랜드 컬러)
    "author": {"name": "{작성자}"},
    "fields": [
      {"name": "Domain", "value": "{domainKey}", "inline": true},
      {"name": "Page",   "value": "{페이지이름 or '-'}", "inline": true}
    ],
    "timestamp": "{ISO8601}",
    "footer": {"text": "Figma comment forwarder"}
  }],
  "allowed_mentions": {"parse": ["roles", "users"]}
}
```

embed 본문 안의 멘션 텍스트는 Discord 가 알림을 발생시키지 않으므로, 멘션은 반드시 외부 `content` 영역에 둔다.

### 분류 → 라우팅 흐름 (Amended)

```
1. comment 조회 (Figma REST)
2. LLM candidates = SELECT domain_key FROM figma_routing_domain
3. ChatCompleteCommand.classify(systemPrompt, userPrompt(comment), candidates)
4. 응답 != null && candidates 에 포함되는가?
   ├─ Yes → SELECT * FROM figma_routing_domain WHERE domain_key = 응답
   └─ No  → SELECT * FROM figma_routing_domain WHERE fallback = true
5. SELECT * FROM figma_routing_domain_mention WHERE domain_id = 매칭 도메인
6. mention 들을 render() → ["<@&123>", "<@456>"]
7. Discord embed payload 빌드 후 webhook 전송
8. last_synced_comment_id 갱신
```
...
> **2026-05-07 Amendment**: 아래 1~10번은 1차 구현(페이지명 기반 라우팅) 의 커밋 계획이며 이미 머지되었다.
...
> 7. `docs: ADR-003 / 업무보고서 LLM 라우팅 기반으로 갱신` (본 변경)
