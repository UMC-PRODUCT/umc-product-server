...
3. **Listener 코드는 변경하지 않는다.** Poller가 outbox row를 처리할 때 내부적으로 Spring
   ...
   현재 `PUBLISHED` 상태는 “local Spring event bus로 예외 없이 디스패치됨”을 의미한다. 기존
   `@TransactionalEventListener(AFTER_COMMIT)` listener의 실제 side effect는 relay transaction commit
   이후 실행되므로, downstream side effect까지 재시도 범위에 넣으려면 listener를 동기 listener 또는
   broker consumer로 별도 전환해야 한다.
