...
#### 1.4 Error / Exception (Sentry SaaS)

- 어댑터: `sentry-spring-boot-starter-jakarta` + `sentry-logback` ([build.gradle.kts §133-135](../../build.gradle.kts#L133-L135))
- DSN: `${SENTRY_DSN}`, traces-sample-rate 기본 0.1 ([application.yml §421-428](../../src/main/resources/application.yml#L421-L428))
- prod profile 에서만 Logback Sentry Appender 가 활성. WARN 이상 이벤트, INFO 이상 breadcrumb. ([logback-spring.xml §93-100](../../src/main/resources/logback-spring.xml#L93-L100))
- Sentry 는 Grafana Cloud 와 **별개의 SaaS** 다. 본 ADR 의 이관 대상 4 종 중 Sentry 는 직접 대체와 부분 통합(Grafana 내 alerting 일원화) 두 갈래의 결정이 필요하다.
...
| `SENTRY_DSN` / `SENTRY_TRACES_SAMPLE_RATE`       | 변동 없음                | 변동 없음 (Sentry 는 별도 ADR)    |
...
- 기존 코드 / 설정
    - [application.yml — management / sentry 섹션](../../src/main/resources/application.yml#L252-L428)
    - [logback-spring.xml — Loki / Sentry 어펜더](../../src/main/resources/logback-spring.xml)
    - [build.gradle.kts — micrometer / opentelemetry / sentry / loki 의존성](../../build.gradle.kts#L127-L141)
