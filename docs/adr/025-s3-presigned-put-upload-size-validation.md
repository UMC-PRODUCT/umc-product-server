...
4. `FileCommandService.confirmUpload`는 실제 S3 객체 정보가 없으면 `FILE_UPLOAD_NOT_COMPLETED`, 크기 초과면 `FILE_SIZE_EXCEEDED`, 요청 크기와 실제 크기가 다르면 `FILE_SIZE_MISMATCH`, Content-Type이 다르면 `INVALID_CONTENT_TYPE`으로 실패시킨다.
...
5. **문서** (`docs/guides`, `src/main/resources/static/docs/catalog/error`): 신규 `FILE_SIZE_MISMATCH` 에러 코드 카탈로그 갱신.
