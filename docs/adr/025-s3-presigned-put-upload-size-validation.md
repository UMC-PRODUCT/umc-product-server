...
이번 결정은 `com.umc.product.storage` 도메인 내부의 AWS S3 업로드 경로에 한정한다. 또한 Hexagonal Architecture 규칙에 따라 애플리케이션 서비스는 `StoragePort`를 통해서만 S3 객체 정보를 조회하고, S3 SDK 의존성은 `adapter.out.s3`에만 둔다.
...
- 인터페이스 default method로 기존 `StoragePort` 호출부의 컴파일 호환성을 유지한다.
