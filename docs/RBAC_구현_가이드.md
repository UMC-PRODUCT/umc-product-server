...
UMC Product Server에서 **RBAC (Role-Based Access Control) + ABAC (Attribute-Based Access Control) 하이브리드** 권한 제어 시스템을 구현하는
방법을 설명합니다.
...
challengerRoles(SCHOOL_PRESIDENT, etc .)
...
hasRole(RoleType.SCHOOL_PRESIDENT)){
        // 특정 학교의 회장 (현재 기수)
        }
```
