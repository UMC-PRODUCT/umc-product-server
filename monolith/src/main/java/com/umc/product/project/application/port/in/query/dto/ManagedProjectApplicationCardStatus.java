 ...
 * <p>
 * 도메인 ENUM ({@link ProjectApplicationStatus}) 에서 임시저장(DRAFT)을 의도적으로 제외한다 -- 본 응답에는 제출 완료된 지원서만 포함하기 때문이다. DRAFT 까지 포함하는
 * {@link ProjectApplicationViewStatus} 와는 의미가 달라 별도 enum 으로 분리한다.
 * <p>
 * 라벨 매핑:
 * <ul>
...
public enum ManagedProjectApplicationCardStatus {
    ...
    /**
     * 도메인 ENUM 으로부터 표시용 status 를 도출한다. DRAFT(임시저장)은 본 enum 에 매핑되지 않으므로 호출자(Repository/Service)가 사전 필터링하지 않은 것은 도메인
