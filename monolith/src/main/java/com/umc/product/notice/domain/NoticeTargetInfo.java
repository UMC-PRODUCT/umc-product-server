 ...
 */
@Schema(description = "공지 대상 범위. null인 필드는 전체 대상. "
    + "예) targetGisuId만 입력하면 해당 기수 전체, targetChapterId도 입력하면 해당 지부만, "
    + "targetParts도 입력하면 해당 파트만, targetNoticeTab이 CHALLENGER가 아니면 운영진 공지")
public record NoticeTargetInfo(
    ...
    @Schema(description = "대상 학교 ID. null이면 전체 학교 대상. 교내 운영진 공지이면 필수", example = "5", nullable = true)
    Long targetSchoolId,
