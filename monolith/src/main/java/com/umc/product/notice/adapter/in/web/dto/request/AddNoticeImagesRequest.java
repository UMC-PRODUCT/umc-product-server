...
public record AddNoticeImagesRequest(
    ...
    @Schema(description = "추가할 이미지 ID 목록. 파일 업로드 API에서 받은 이미지 ID를 전달")
    @NotEmpty(message = "이미지 ID 리스트는 비어 있을 수 없습니다.") List<String> imageIds
) {

    public AddNoticeImagesCommand toCommand() {
        return new AddNoticeImagesCommand(imageIds);
    }
}
