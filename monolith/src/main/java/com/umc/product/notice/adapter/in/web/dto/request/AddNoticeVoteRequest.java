...
public record AddNoticeVoteRequest(
    ...
    @NotBlank(message = "투표 제목은 필수입니다.") String title,

    @Schema(description = "익명 투표 여부")
    @NotNull(message = "익명 투표 여부는 필수입니다.") boolean isAnonymous,

    @Schema(description = "복수 선택 허용 여부 (false=단일선택, true=복수선택)")
    @NotNull(message = "복수 선택 허용 여부는 필수입니다.") boolean allowMultipleChoice,

    @Schema(description = "투표 시작 시각")
    @NotNull(message = "투표 시작 시각은 필수입니다.") Instant startsAt,

    @Schema(description = "투표 마감 시각 (exclusive)")
    @NotNull(message = "투표 마감 시각은 필수입니다.") Instant endsAtExclusive,

    @Schema(description = "투표 선택지 목록 (2~5개)")
    @NotEmpty(message = "투표 선택지는 필수입니다.") List<String> options
) {
