...
/**
 * 공개 destination 구독자 또는 인증 사용자에게 메시지를 전달하기 위한 Port Out.
 * <p>
 ...
 * <p>
 * 실제 전송 메커니즘(STOMP simple broker, user destination, 외부 broker relay 등)은 어댑터 구현체가
 * 담당한다.
 */
public interface BroadcastPort {
    ...
    /**
     * 인증된 사용자에게 payload를 전송한다.
     *
     * @param userName   {@link java.security.Principal#getName()}과 일치하는 사용자 식별자
     * @param destination client의 {@code /user} prefix를 제외한 destination
     * @param payload    전송할 payload
     */
    void broadcastToUser(String userName, String destination, Object payload);
}
