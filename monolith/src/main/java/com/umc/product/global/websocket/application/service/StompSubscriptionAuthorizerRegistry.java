...
/**
 * STOMP SUBSCRIBE destination을 소유한 소비 도메인 authorizer에 인가를 위임한다.
 * <p>
 * destination을 지원하는 authorizer가 없거나 둘 이상이면 소유권이 불명확하므로 fail-closed 처리한다.
 */
@Service
@RequiredArgsConstructor
public class StompSubscriptionAuthorizerRegistry {

    private final List<StompSubscriptionAuthorizer> authorizers;
    ...
    public boolean isAuthorized(Long memberId, String destination) {
        ...
        List<StompSubscriptionAuthorizer> matchedAuthorizers = authorizers.stream()
