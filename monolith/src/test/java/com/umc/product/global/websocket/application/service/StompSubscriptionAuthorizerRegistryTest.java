...
@DisplayName("StompSubscriptionAuthorizerRegistry")
class StompSubscriptionAuthorizerRegistryTest {

    private static final String DESTINATION = "/topic/test/rooms/10/messages";

    @Test
    @DisplayName("destination을 지원하는 authorizer 하나가 승인하면 구독을 허용한다")
    void authorize_singleAuthorizerAllows() {
        StompSubscriptionAuthorizerRegistry sut = new StompSubscriptionAuthorizerRegistry(
    ...
    @Test
    @DisplayName("destination을 지원하는 authorizer가 없으면 구독을 거부한다")
    ...
    @Test
    @DisplayName("destination을 지원하는 authorizer가 거부하면 구독을 거부한다")
    ...
    @Test
    @DisplayName("destination을 지원하는 authorizer가 둘 이상이면 구독을 거부한다")
    ...
    @Test
    @DisplayName("인증 주체나 destination이 없으면 구독을 거부한다")
    void authorize_missingInputDenies() {
        StompSubscriptionAuthorizerRegistry sut = new StompSubscriptionAuthorizerRegistry(List.of());

        assertThat(sut.isAuthorized(null, DESTINATION)).isFalse();
        assertThat(sut.isAuthorized(10L, null)).isFalse();
    }

    private StompSubscriptionAuthorizer authorizer(boolean supports, boolean authorized) {
