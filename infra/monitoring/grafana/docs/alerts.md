...
| `HighDiskUsage`      |  warning | 10m | Disk usage > 80%                    | `instance`, `mountpoint` |
| `HighDiskUsage`      | critical | 10m | Disk usage > 90%                    | `instance`, `mountpoint` |
| `LowDiskFreeSpace`   |  warning | 10m | Disk free space < 10GB              | `instance`, `mountpoint` |
...
| Alert                          | Severity | For | 발생 조건                                  | 주요 라벨                      |
| ------------------------------ | -------: | --: | ------------------------------------------ | ------------------------------ |
| `ErrorLogsSpike`               |  warning |  3m | 최근 5분 `ERROR` log > 10                  | `service_name`, `alert_source` |
| `StructuredErrorLogsSpike`     |  warning |  3m | 최근 5분 structured `level=ERROR` log > 10 | `service_name`, `alert_source` |
| `EmailDomainExceptionSpike`    |  warning |  3m | 최근 5분 `EmailDomainException` > 3        | `service_name`, `alert_source` |
| `NullPointerExceptionDetected` |  warning |  0m | `NullPointerException` log 감지            | `service_name`, `alert_source` |
| `StructuredLoginFailuresSpike` |  warning |  3m | 최근 5분 `auth_login_failed` event > 5     | `service_name`, `alert_source` |
| `EmailSendFailures`            |  warning |  0m | 최근 5분 email 관련 failure log > 3        | `service_name`, `alert_source` |
| `StructuredEmailSendFailures`  |  warning |  0m | `email_send_failed` event 감지             | `service_name`, `alert_source` |
