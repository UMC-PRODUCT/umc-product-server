-- 1. submission 테이블 생성
CREATE TABLE challenger_workbook_submission
(
    ...
    CONSTRAINT pk_challenger_workbook_submission PRIMARY KEY (id),
    CONSTRAINT uk_challenger_workbook_submission_challenger_workbook_id UNIQUE (challenger_workbook_id),
    CONSTRAINT fk_challenger_workbook_submission_challenger_workbook
        FOREIGN KEY (challenger_workbook_id) REFERENCES challenger_workbook (id)
);

-- 2. review 테이블 생성
CREATE TABLE challenger_workbook_review
(
    id            BIGSERIAL                   NOT NULL,
    ...
    created_at    TIMESTAMP(6) WITH TIME ZONE NOT NULL,
    updated_at    TIMESTAMP(6) WITH TIME ZONE NOT NULL,
    CONSTRAINT pk_challenger_workbook_review PRIMARY KEY (id),
    CONSTRAINT fk_challenger_workbook_review_submission
        FOREIGN KEY (submission_id) REFERENCES challenger_workbook_submission (id),
...
INSERT INTO challenger_workbook_submission (challenger_workbook_id, content, created_at, updated_at)
SELECT id, submission, created_at, updated_at
FROM challenger_workbook
WHERE status != 'PENDING';

-- 3-2. 심사 완료된 워크북(PASS, FAIL, BEST)의 리뷰 기록 마이그레이션
INSERT INTO challenger_workbook_review (submission_id, reviewer_challenger_id, feedback, status, best_reason, created_at, updated_at)
SELECT s.id, 0, cw.feedback, cw.status, cw.best_reason, cw.created_at, cw.updated_at
FROM challenger_workbook cw
JOIN challenger_workbook_submission s ON s.challenger_workbook_id = cw.id
