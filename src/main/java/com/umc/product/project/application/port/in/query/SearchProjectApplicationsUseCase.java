 ...
 * <p>
 * 본 UseCase 는 ProjectApplication 자원만 반환한다. 화면 카드 조립에 필요한 부가 정보
 * (지원자 챌린저 part / 매칭 라운드 정보 / 지원자 닉네임/실명/학교)는 Web Assembler 가 cross-domain UseCase 를 통해 합성한다.
 * 파트(part) 필터도 챌린저 도메인의 정보가 필요하므로 Assembler 단계에서 in-memory 로 적용한다.
 * <p>
