 ...
 * 의 시맨틱을 흔들지 않는다.
 * <p>
 * 본 enum 은 임시저장(PENDING)을 그대로 포함한다. 따라서 PENDING 노출이 허용되는 호출 시점에서 사용하며, 현재 사용처는 다음과 같다.
 * <ul>
 ...
 * </ul>
 * <p>
 * PENDING 을 절대 노출해서는 안 되는 PM/운영진 지원자 목록 응답에는 본 enum 대신
 * {@link ManagedProjectApplicationCardStatus} 를 사용한다 — 두 enum 은 PENDING 포함 여부에서 의도적으로 다르므로 혼용하지 않는다.
 */
