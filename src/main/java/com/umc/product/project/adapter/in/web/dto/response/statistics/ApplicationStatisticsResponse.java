...
public record ApplicationStatisticsResponse(
    ...
    @Schema(description = "학교별 지원 집계(차수 목록 중첩). schoolStats[].total은 운영진만 제공(PM챌린저는 null).") List<SchoolStat> schoolStats,
    @Schema(description = "프로젝트별 지원 집계(차수 목록 중첩). 운영진 전용. PM챌린저 호출 시 null.", nullable = true) List<ProjectRoundStat> projectRoundStats
) {
