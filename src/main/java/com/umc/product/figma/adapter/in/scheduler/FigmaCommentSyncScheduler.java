 ...
 * <p>
 * 매 사이클 figma_summary_cursor 의 last_window_end 를 from 으로, 현재 시각을 to 로 하는 시간창으로 SummarizeFigmaCommentsUseCase 를 호출한다. cursor 가
 * 부재한 부트스트랩 환경에서는 안전 fallback 으로 (now - pollInterval × 2, now] 를 사용한다 — 첫 사이클이 직전 짧은 구간만 보더라도 dispatch dedup 으로 중복이 막힌다.
 * <p>
 * 다중 인스턴스 환경에서 두 인스턴스가 동시에 트리거되면 둘 다 같은 시간창을 처리하지만, dispatch unique(comment_id) 제약과 cursor 의 방어적 advance 로 중복 발송이 차단된다 (LLM/Figma
 * REST 호출이 도중에 잠시 중복될 수 있다는 trade-off 가 남는다 — ShedLock 도입은 후속 작업).
 */
@Slf4j
@Component
