 ...
 * <p>
 * 시간창 시맨틱으로 옮기면서 사라진 "이미 발송된 댓글을 다시 보내지 않는다" 가드 책임을
 * 본 엔티티가 담당한다. (comment_id) 에 unique 제약이 걸려 있어 같은 commentId 가
 * 두 도메인 묶음에 동시에 들어가는 사고를 DB 단에서도 막는다.
 * <p>
 * admin digest 는 force=true 로 본 테이블을 무시하고 재발송할 수 있다.
