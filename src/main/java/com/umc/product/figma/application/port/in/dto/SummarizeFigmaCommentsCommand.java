 ...
 * <p>
 * 세 진입점 (스케줄러 sync / admin digest / admin preview) 은 본 record 의 정적 팩토리로 정해진 플래그 조합을 사용해 동일한 본체
 * ({@code FigmaCommentSummaryService}) 를 호출한다.
 *
 * @param from          시간창 시작 (inclusive). null 이면 무한 과거.
 * @param to            시간창 끝 (inclusive). null 이면 무한 미래.
 * @param singleFileId  null 이면 활성 watched file 전체를 대상으로 한다. 비어있지 않으면 해당 파일 1건만 대상으로 하며, enabled 여부와 무관하게 처리한다 (운영진
 *                      on-demand 트리거 / preview 용).
 * @param dryRun        true → Discord 발송 / dispatch 기록 / cursor advance 모두 건너뛰고 묶음 결과만 반환. preview 진입점이 사용한다.
 * @param force         true → dispatch 행이 있는 commentId 도 발송 대상에 포함한다. admin digest 가 사용한다 (catch-up / 회고용).
 * @param advanceCursor true → 발송이 완료된 후 figma_summary_cursor.last_window_end 를 to 로 advance. 스케줄러 sync 진입점만 사용한다.
 */
