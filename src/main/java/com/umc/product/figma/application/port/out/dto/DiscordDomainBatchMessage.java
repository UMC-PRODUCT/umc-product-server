 ...
 *
 * 한 도메인 = 한 webhook = 한 메시지(embeds 다중)로 합쳐 발송하기 위한 DTO 다.
 * 같은 도메인의 댓글이 25건을 넘으면 어댑터가 embed 를 추가로 분할하고 (Discord embed 는
 * fields 25개 / 메시지 당 embed 10개 / 전체 6000자 제한이 있음), 그래도 초과하면
 * 추가 메시지로 페이지네이션한다.
 *
 * @param webhookUrl     도메인의 Discord webhook URL
 * @param domainKey      라우팅 도메인 식별자 (embed title / footer 에 노출)
 * @param mentionRenders 미리 렌더된 멘션 문자열 (예: ["<@&123>", "<@456>"])
 *                       — 알림 발생을 위해 메시지의 외부 content 영역에 출력된다.
