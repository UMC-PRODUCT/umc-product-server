...
public class ScheduleQueryRepository {
     ...
     * <p>
     * [조건]
     * - attendance_sheet.gisu_id = :gisuId (현재 기수)
     * - attendance_sheet.requires_approval = true (승인 필요 출석부)
     * - attendance_record.member_id = :memberId (본인이 출석 대상인 일정)
     * <p>
     * [참고]
     * - 일정 생성자는 무조건 attendance_record에 포함되므로, 본인 생성 일정도 자동으로 조회됨
     * - 따라서 별도의 author_challenger_id 조건 불필요
     */
     ...
     * <p>
     * [조건]
     * - attendance_sheet.requires_approval = true
     * - attendance_sheet.gisu_id = :gisuId
     * - (member.school_id = :schoolId OR schedule.author_challenger_id = :authorChallengerId)
     *   → 파트장이 본인 학교 소속이거나, 본인이 생성한 일정
     * <p>
     * [leftJoin 사용 이유]
     * - schedule.study_group_id가 nullable → 스터디 그룹 없는 일정 존재
     * - 본인 생성 일정은 스터디 그룹과 무관하게 조회되어야 함
     ...
     * - attendance_sheet.requires_approval = true
     * - attendance_sheet.gisu_id = :gisuId
     * - ((study_group_member.challenger_id = :challengerId AND is_leader = true)
     *    OR schedule.author_challenger_id = :challengerId)
     *   → 본인이 파트장인 스터디 일정이거나, 본인이 생성한 일정
     * <p>
     * [leftJoin 사용 이유]
     * - schedule.study_group_id가 nullable → 스터디 그룹 없는 일정 존재
     * - 본인 생성 일정은 스터디 그룹과 무관
     * - leftJoin으로 스터디 그룹이 없어도 본인 생성 일정 조회 가능
     * <p>
     * [주의]
     * - study_group_member 조인 시 is_leader 조건을 ON절이 아닌 WHERE절에서 처리
     * - 모든 멤버와 조인 후 WHERE에서 파트장 여부 필터링
     */
     ...
     * - attendance_sheet.requires_approval = true
     * - attendance_sheet.gisu_id = :gisuId
     * - schedule.author_challenger_id = :authorChallengerId
     *   → 본인이 생성한 일정만 조회
     * <p>
     * [적용 대상]
     * - 중앙운영사무국원, 학교 회장단, 학교 파트장이 아닌 기타 운영진
     * - SCHOOL_ETC_ADMIN 등의 역할
     */
