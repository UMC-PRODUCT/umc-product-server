...
public class RecruitmentDocumentEvaluationService implements UpdateMyEvaluationUseCase,
        CreateMyEvaluationUseCase,
        UpdateDocumentStatusUseCase {

    @Override
    public MyEvaluationInfo update(UpdateMyEvaluationCommand command) {
    ...
    @Override
    public MyEvaluationInfo create(CreateMyEvaluationCommand command) {
        // todo: 평가 기간, 운영진 권한 검증 필요
        // 매번 운영진의 점수를 모두 조회해와서 평균을 내는 방식은 비효율적일 수 있으므로, 지원서 평가의 평균 점수를 지원서 엔티티에 별도로 저장해두고,
        // 평가가 등록/수정될 때마다 해당 값을 갱신하는 방식을 고려 중에 있습니다.
        return null;
