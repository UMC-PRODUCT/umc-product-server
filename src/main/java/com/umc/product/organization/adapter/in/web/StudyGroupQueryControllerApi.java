 ...
 *   → 파트 선택
 *
 * GET /?gisuId=9&schoolId=1&part=WEB&cursor=&size=20
 *   → 스터디 그룹 목록 (무한스크롤)
 *   → 그룹 선택
 *
 * GET /{groupId}
 ...
 *   → 파트 선택 (자동)
 *
 * GET /?gisuId=9&schoolId=1&part={본인파트}&cursor=&size=20
 *   → 스터디 그룹 목록 (무한스크롤)
 *
 * GET /{groupId}
 *   → 스터디 그룹 상세
 ...
 *   → 파트 선택
 *
 * GET /?gisuId=9&schoolId={본인학교}&part=WEB&cursor=&size=20
 *   → 스터디 그룹 목록 (무한스크롤)
 *
 * GET /{groupId}
 *   → 스터디 그룹 상세
 ...
 * <h3>4. 파트장</h3>
 * <pre>
 * GET /?gisuId=9&schoolId={본인학교}&part={본인파트}&cursor=&size=20
 *   → 스터디 그룹 목록 (무한스크롤)
 *
 * GET /{groupId}
 *   → 스터디 그룹 상세
...
public interface StudyGroupQueryControllerApi {
    ...
    @Operation(
            summary = "1단계: 학교 목록 조회",
            description = "스터디 그룹이 있는 학교 목록을 조회합니다. 중앙 운영진, 중앙 파트장용."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
                    content = @Content(schema = @Schema(implementation = StudyGroupSchoolsResponse.class))
            )
    })
    ...
    @Operation(
            summary = "2단계: 파트 목록 조회",
            description = "특정 학교의 파트별 스터디 그룹 요약을 조회합니다."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
                    content = @Content(schema = @Schema(implementation = StudyGroupPartsResponse.class))
            ),
    ...
    @Operation(
            summary = "3단계: 스터디 그룹 목록 조회",
            description = "특정 학교, 파트의 스터디 그룹 목록을 조회합니다. cursor 기반 무한스크롤."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "조회 성공")
    })
    CursorResponse<StudyGroupSummary> getStudyGroups(
            @Parameter(description = "기수 ID", required = true) Long gisuId,
            @Parameter(description = "학교 ID", required = true) Long schoolId,
    ...
    @Operation(
            summary = "4단계: 스터디 그룹 상세 조회",
            description = "스터디 그룹의 상세 정보와 멤버 목록을 조회합니다."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
