...
public record StudyGroupPartsResponse(@Schema(description = "학교 ID", example = "1") Long schoolId,
    ...
    public static StudyGroupPartsResponse from(PartSummaryInfo info) {
        return new StudyGroupPartsResponse(info.schoolId(), info.schoolName(), Part.fromList(info.parts()));
    }

    @Schema(description = "파트 요약 정보")
    public record Part(@Schema(description = "파트", example = "WEB") String part,
        ...
        public static Part from(PartSummaryInfo.PartInfo p) {
            return new Part(p.part().name(), p.part().getDisplayName(), p.studyGroupCount(), p.memberCount());
        }
