...
public record AddStudyGroupMembersRequest(
        @Schema(description = "스터디원 챌린저 ID 목록", example = "[101, 102, 103]")
        @NotNull(message = "챌린저 ID 목록은 필수입니다")
        Set<Long> challengerIds
) {
    public AddStudyGroupMembersCommand toCommand(Long groupId) {
        return new AddStudyGroupMembersCommand(groupId, challengerIds);
    }
}
