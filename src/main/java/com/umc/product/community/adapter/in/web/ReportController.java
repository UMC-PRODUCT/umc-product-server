...
import com.umc.product.community.adapter.in.web.dto.response.ReportResponse;
...
import com.umc.product.community.application.port.in.report.query.GetReportListUseCase;
import com.umc.product.community.application.port.in.report.query.ReportInfo;
...
public class ReportController {
    ...
    @GetMapping("/reports")
    @Operation(summary = "신고 목록 조회 (관리자)", description = "모든 신고 내역을 조회합니다. (관리자 전용)")
    public PageResponse<ReportResponse> getReports(
            @PageableDefault(size = 20)
            @ParameterObject
            Pageable pageable
    ) {
        Page<ReportInfo> reports = getReportListUseCase.getReports(pageable);
        return PageResponse.of(reports, ReportResponse::from);
    }
}
