...
public class CurriculumQueryService implements GetCurriculumUseCase {
    ...
    @Override
    public MyCurriculumInfo getMyProgress(Long memberId, Long gisuId, ChallengerPart part) {
        ChallengerInfo challengerInfo = getChallengerUseCase.getByMemberIdAndGisuId(memberId, gisuId);
        ChallengerPart selected = resolvePart(challengerInfo, part);
        CurriculumProjection projection = loadCurriculumPort.getByGisuIdAndPart(gisuId, selected);
