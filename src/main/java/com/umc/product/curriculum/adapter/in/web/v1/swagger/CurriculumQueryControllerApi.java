...
public interface CurriculumQueryControllerApi {
    ...
    @Operation(
        ...
        description = """
            ...
            현재 활성화된 기수의 파트별 커리큘럼과 워크북 목록을 조회합니다.\s\s
    ...
    @Operation(
        ...
        description = """
            ...
            챌린저의 커리큘럼 진행 상황을 조회합니다.\s\s
            각 주차별 워크북의 상태(기본/진행중/제출완료/통과/실패)를 반환합니다.\s\s

            현재 활성 기수 기준으로만 조회되어 이전 기수 사용자는 조회가 불가합니다. \s\s
            """,
        deprecated = true
    )
    ...
    @Operation(
        ...
        description = """
            ...
            파트를 기준으로 활성 기수의 커리큘럼 주차 목록을 조회합니다.\s\s
            각 주차의 번호와 제목만 반환합니다.\s\s

            `GET /api/v2/curriculums/{gisuId}?part={part}` 응답의 `workbooks` 필드에 주차 정보가 포함되어 있습니다.\s\s
