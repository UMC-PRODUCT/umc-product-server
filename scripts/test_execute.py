...
def tmp_project(tmp_path):
    """phases/, CLAUDE.md, docs/harness/ 를 갖춘 임시 프로젝트 구조."""
    ...
    (harness_dir / "ARCHITECTURE.md").write_text("# Architecture\nSome content")
    (harness_dir / "ADR.md").write_text("# ADR\nAnother doc")
...
class TestLoadGuardrails:
    def test_loads_claude_md_and_harness_docs(self, executor, tmp_project):
        with patch.object(ex, "ROOT", tmp_project):
    ...
    def test_docs_sorted_alphabetically(self, executor, tmp_project):
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        adr_pos = result.index("ADR")
        arch_pos = result.index("Architecture")
        assert adr_pos < arch_pos
    ...
    def test_no_harness_dir(self, executor, tmp_project):
        import shutil
        shutil.rmtree(tmp_project / "docs" / "harness")
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        assert "Rules" in result
