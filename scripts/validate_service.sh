...
# Configurable environment variables (defaults provided)
...
HEALTH_URL=${HEALTH_URL:-http://localhost:${PORT}${HEALTH_PATH}}
...
health_check() {
    ...
    while [ "$attempt" -le "$max_attempts" ]; do
        RESPONSE=$(curl -s -w "\n%{http_code}" --max-time 5 "${HEALTH_URL}" 2>/dev/null || echo "")
...
if health_check; then
    ...
    echo ""
    echo "=== 프로세스 정보 ==="
    pgrep -fa "${JAR_NAME}" || echo "프로세스 정보를 찾을 수 없습니다"
else
    echo "💥 헬스체크 실패!"
    exit 1
fi
